...
use risingwave_sqlparser::ast::{
    self, Function, FunctionArg, FunctionArgExpr, WindowFrameBound, WindowFrameExclusion,
        ...
        match (kind, direct_args.as_mut_slice(), args.as_mut_slice()) {
            (AggKind::PercentileCont | AggKind::PercentileDisc, [fraction], [arg]) => {
                if fraction.cast_implicit_mut(DataType::Float64).is_ok()
                    && let Some(Ok(casted)) = fraction.try_fold_const() {
                    ...
                    *fraction = Literal::new(casted, DataType::Float64).into();
                    ...
                    return Err(ErrorCode::InvalidInputSyntax(format!(
                        "direct arg in `{}` must be constant and castable to float64",
                        kind
                    ))
                    .into());
                }
    ...
    pub(super) fn bind_window_function(
        ...
        let frame = if let Some(frame) = window_frame {
            ...
            let bounds = match frame.units {
                ...
                //     let (start, end) = self
                //         .bind_window_frame_scalar_impl_bounds(frame.start_bound,
                // frame.end_bound)?;     // TODO()
    ...
    fn bind_window_frame_bound_offset(
        ...
        let Some(Ok(offset)) = offset.try_fold_const() else {
            return Err(ErrorCode::InvalidInputSyntax(format!(
                ...
                cast_to.map(|t| format!(" and castable to {}", t)).unwrap_or_default()
            ))
            .into());
        };
