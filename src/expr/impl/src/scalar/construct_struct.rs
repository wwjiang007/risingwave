...
fn build(return_type: DataType, children: Vec<BoxedExpression>) -> Result<BoxedExpression> {
    ...
    return_type.as_struct().types().zip_eq(children.iter()).for_each(|(ty, child)| {
...
mod tests {
    ...
    async fn test_construct_struct_expr() {
        let expr = build_from_pretty("(construct_struct:struct<a_int4,b_int4,c_int4> $0:int4 $1:int4 $2:int4)");
